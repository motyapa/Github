...
public class TestCalculator extends BaseTest {
    ...
    public void basicTestToSimplify() {
        ...
        assertEquals("5 * a + 7", calc.evaluate("(3 + 2) * a + (4 + 3)"));
        assertEquals("3 * x + a", calc.evaluate("3 * x + a"));
        assertEquals("3", calc.evaluate("x := 3"));
        assertEquals("9 + a", calc.evaluate("3 * x + a"));
    ...
    public void testBasicOperators() {
        ...
        assertEquals("-7", calc.evaluate("((3 + 4) * (1 - 2))"));
        assertEquals("-7", calc.evaluate("-(3 + 4)"));
        assertEqualsOneOf(
                new String[] {"3 * -4", "-12"},
                calc.evaluate("3 * -4"));
